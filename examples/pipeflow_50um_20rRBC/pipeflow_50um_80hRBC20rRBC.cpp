...
int main(int argc, char *argv[]) {
  ...
  hemocell.addCellType<RbcHighOrderModel>("hRBC", RBC_FROM_SPHERE);
  hemocell.setMaterialTimeScaleSeparation("hRBC", (*cfg)["ibm"]["stepMaterialEvery"].read<int>());
  hemocell.setInitialMinimumDistanceFromSolid("hRBC", 0.5); //Micrometer! not LU


  hemocell.addCellType<RbcHighOrderModel>("rRBC", RBC_FROM_SPHERE);
  hemocell.setMaterialTimeScaleSeparation("rRBC", (*cfg)["ibm"]["stepMaterialEvery"].read<int>());
  hemocell.setInitialMinimumDistanceFromSolid("rRBC", 0.5); //Micrometer! not LU
  ...
  hemocell.setOutputs("hRBC", outputs);
  hemocell.setOutputs("rRBC", outputs);
  ...
  while (hemocell.iter < tmax ) {
    ...
    if (hemocell.iter % tmeas == 0 || hemocell.iter == 100) {
        ...
        hlog << " | # of hRBC: " << CellInformationFunctionals::getNumberOfCellsFromType(&hemocell, "hRBC");
	hlog << " | # of rRBC: " << CellInformationFunctionals::getNumberOfCellsFromType(&hemocell, "rRBC");
        ...
        ParticleStatistics pvinfo = ParticleInfo::calculateVelocityStatistics(&hemocell);
        hlog << "\t Particle velocity  -  max.: " << pvinfo.max * toMpS << " m/s, mean: " << pvinfo.avg * toMpS << " m/s" << endl;
